...
describe('subpath imports', () => {
	it('throws when no imports field', () => {
		expect(() => resolve(manifest({}), '#ansi-styles')).toThrow(
			'Package import specifier "#ansi-styles" is not defined in package path/to/package/package.json imported from working/dir'
		)
	})

	it('throws when no match', () => {
		expect(() => resolve(manifest({ imports: {} }), '#ansi-styles')).toThrow(
			'Package import specifier "#ansi-styles" is not defined in package path/to/package/package.json imported from working/dir'
		)
	})

	it('throws when import key does not start with #', () => {
		expect(() => resolve(manifest({ imports: { x: 'y' } }), '#x')).toThrow(
			'Package import specifier "#x" is not defined in package path/to/package/package.json imported from working/dir'
		)
	})
